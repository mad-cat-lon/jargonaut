from jargonaut.transformations.data.mba_utils import constant_to_linear_mba
import random 
...
def generate_static_mba_eq_pred(as_obj=True, int_names=[]):
    ...
    if int_names:
        left_mba_expr = constant_to_linear_mba(pred_int, n_terms=random.randint(4, 7), as_obj=False)
        ...
        right_mba_expr = constant_to_linear_mba(pred_int, n_terms=random.randint(4, 7), as_obj=False)
...
def generate_static_mba_mod_pred(as_obj=True, int_names=[]):
    ...
    print(f"{a} % {n} == {r}")    
    if int_names:
        a_mba_expr = constant_to_linear_mba(a, n_terms=random.randint(4, 7), as_obj=False)
        ...
        n_mba_expr = constant_to_linear_mba(n, n_terms=random.randint(4, 7), as_obj=False)
        ...
        r_mba_expr = constant_to_linear_mba(r, n_terms=random.randint(4, 7), as_obj=False)
