...
def binary(keys, string):
    ...
    vars = [''.join(random.choice(alphabet.upper()+alphabet.lower()) for i in range(5)) for j in range(5)]
    ...
    to_bin_func = f"(''.join(['0' if {vars[4]} == '{keys[0]}' else '1' for {vars[4]} in '{obfus}']))"
